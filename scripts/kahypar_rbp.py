...
# SETUP ENV
###################################
algorithm = "KaHyPar-RBP"
kahypar_rbp = os.environ.get("KAHYPAR_BP")
kahypar_rbp_config = os.environ.get("KAHYPAR_RBP_CONFIG")
assert (kahypar_rbp != None and kahypar_rbp_config != None), "check env.sh"
###################################
...
# Run KaHyPar-K
kahypar_rbp_proc = subprocess.Popen([kahypar_rbp,
                                   "-h" + args.graph,
                                   "-k" + str(args.k),
                                   "-e" + str(args.epsilon),
                                   ...
                                   "-mrecursive",
                                   "-p" + kahypar_rbp_config,
                                   "--sp-process=true"],
                                  stdout=subprocess.PIPE, universal_newlines=True)

def kill_proc():
	kahypar_rbp_proc.terminate() #signal.SIGTERM

t = Timer(args.timelimit, kill_proc)
t.start()
out, err = kahypar_rbp_proc.communicate()
t.cancel()
end = time.time()
...
if kahypar_rbp_proc.returncode == 0:
...
elif kahypar_rbp_proc.returncode == -signal.SIGTERM:
  timeout = "yes"
else:
  failed = "yes"
